...
impl<S> FileDb<S> where S: Clone + Serialize + DeserializeOwned + MutableState,
                        S::Mutation: Clone,
                        S::MutateError: Debug,
{
    /// Create a new database file from an initial state
    /// Aborts if destination file already exists
    pub fn create(path_buf: PathBuf, initial_state: S) -> Result<Self, FileDbError<S::MutateError>> {
        if path_buf.exists() {
            return Err(FileDbError::FileAlreadyExists);
        }
        // There is no file, we create a new file
        // Serialize the state
        let ser_string = serde_json::to_string_pretty(&initial_state)
            .map_err(FileDbError::SerdeJsonError)?;
    ...
    pub fn load(path_buf: PathBuf) -> Result<Self, FileDbError<S::MutateError>> {
        ...
        f.read_to_string(&mut ser_string).map_err(FileDbError::ReadError)?;
        let state: S = serde_json::from_str(&ser_string).map_err(FileDbError::SerdeJsonError)?;
        Ok(FileDb { path_buf, state })
    }
}

impl<S> AtomicDb for FileDb<S> where S: Debug + Clone + Serialize + DeserializeOwned + MutableState,
                                     S::Mutation: Clone,
                                     S::MutateError: Debug,
{
    type State = S;
    ...
    fn mutate_db(&mut self, mutations: &[Self::Mutation]) -> Result<(), Self::Error> {
        ...
        for mutation in mutations.iter() {
            self.state.mutate(mutation).map_err(FileDbError::MutateError)?;
        }
        // Serialize the state
        let ser_string = serde_json::to_string_pretty(&self.state).map_err(FileDbError::SerdeJsonError)?;
        // Save the new state to file, atomically
        let af = atomicwrites::AtomicFile::new(&self.path_buf, atomicwrites::AllowOverwrite);
        af.write(|fw| fw.write_all(ser_string.as_bytes())).map_err(FileDbError::WriteError)?;
        Ok(())
    }
}
...
mod tests {
    ...
    impl DummyState {
        pub fn new(x: u32) -> Self { DummyState { x } }
    }
    ...
    fn test_file_db_basic() {
        ...
        file_db.mutate_db(&[DummyMutation::Inc, DummyMutation::Inc, DummyMutation::Dec]).unwrap();
